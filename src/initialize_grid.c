...
void write_JSON_onStruct(   gridConfiguration *gridCfg, 
    ...
    cJSON *Filename_Config = cJSON_GetObjectItemCaseSensitive(json, "filename_Configuration");   //filename hdf5
    if( cJSON_IsString(Filename_Config) && (Filename_Config->valuestring != NULL) ){
        file_config = strdup(Filename_Config->valuestring);
    }
    ...
    if( cJSON_IsNumber(item_boundary) ){
        boundary_sel = item_boundary->valueint;
    }  
...
void control_gridInit(  gridConfiguration *gridCfg, 
    ...
    ALLOC_3D(G->EB_WAVE_ref, Nx, Ny, Nz_ref, double);
    ALLOC_3D(G->J_B0, Nx, Ny, Nz, double);
    ALLOC_3D(G->n_e, Nx/2, Ny/2, Nz/2, double);

    /*Initialize saveData matrices*/
    ALLOC_3D(saveDCfg->data2save, Nx/2, Ny/2, Nz/2, double);
    ALLOC_2D(saveDCfg->timetraces, t_end/(int)period, 8, double );
